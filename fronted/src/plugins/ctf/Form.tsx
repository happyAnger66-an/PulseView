...
export default function CtfDatasourceForm({ action, data, onFinish, submitLoading }: DatasourceFormProps) {
  ...
  const handleInspect = async () => {
    const path = form.getFieldValue(['settings', 'ctf.path']);
    if (!path) {
      setInspectError('请先填写 CTF trace 目录路径');
      return;
    }
    setInspecting(true);
    setInspectError('');
    ...
    <Form form={form} layout='vertical' initialValues={data} onFinish={onFinish}>
      <Card title={`${action === 'add' ? '新增' : '编辑'} ${PLUGIN_NAME} 数据源`}>
        <Alert
          ...
          message='指向 CTF trace 目录（含 metadata 与 stream_*）。回调区间将转为 span 表，支持 Timeline 泳道视图。'
        />
        <Form.Item name='name' label='名称' rules={[{ required: true, message: '请输入名称' }]}>
          <Input placeholder='例如 ros2-trace-001' />
        </Form.Item>
        <Form.Item label='CTF trace 目录' required>
          <Space.Compact style={{ width: '100%' }}>
            <Form.Item
              name={['settings', 'ctf.path']}
              noStyle
              rules={[{ required: true, message: '请输入 CTF trace 目录' }]}
            >
              <Input placeholder='/path/to/ctf_sample' />
            </Form.Item>
