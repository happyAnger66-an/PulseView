...
class TopicTracer {
 public:
  template <
      class MsgType,
      std::enable_if_t<!mw::system_stats::has_input_infos<MsgType>::value,
                       bool> = true>
  ...
  int32_t AppendInputTopicInfos(const std::string & /*topic*/, MsgType *msg) {
    if (!msg->has_header()) {
      ...
      add_header->set_timestamp_ns(mw_base::NowNs());
    }
  ...
  template <class MsgType,
            std::enable_if_t<!mw::system_stats::has_header<MsgType>::value,
                             bool> = true>
 ...
 private:
  ...
  int add_input_recv_timestamp(mw::common::InputInfo *msg) {
