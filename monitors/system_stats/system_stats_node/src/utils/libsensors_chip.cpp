...
void TempSensor::buildStatus(mw::system_stats::OutputData &msg) {
  ...
  if (temp) {
    mw::system_stats::OutputCpuTemp cpu_temperature;
    cpu_temperature.description = temp->getFeature().getFeatureLabel();
    cpu_temperature.temperature = temp->getValue();
    if (max_temp && max_temp->getValue() != 0)
      cpu_temperature.high = max_temp->getValue();
    if (temp_crit && temp_crit->getValue() != 0)
      cpu_temperature.crit = temp_crit->getValue();
