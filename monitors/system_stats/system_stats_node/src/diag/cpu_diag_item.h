#pragma once
#include <cstdint>
#include <memory>
#include <string>
#include <unordered_map>

#include "diag/diag_item.h"

namespace mw {
namespace system_stats {

class CpuDiagItem : public DiagItem {
 public:
  CpuDiagItem() : DiagItem() {}
  ~CpuDiagItem() = default;

  int32_t Start(const mw::proto::SystemStatsDiagConfig &cfg) override;

  int32_t Stop() override;

  int32_t Diagnose(
      const OutputData &data,
      std::vector<diagnostic_msgs::msg::DiagnosticStatus> &status_vec) override;

 private:
  void node_defaul_cpu_diag(
      const OutputNodeInfo &node_cpu,
      std::vector<diagnostic_msgs::msg::DiagnosticStatus> &status_vec);
  bool node_cpu_diag(
      const OutputNodeInfo &node_cpu,
      std::vector<diagnostic_msgs::msg::DiagnosticStatus> &status_vec);
  struct CpuDiagItemConfig {
    float warn = 0.0;
    float error = 0.0;
  };

  CpuDiagItemConfig system_cpu_;
  CpuDiagItemConfig default_cpu_;
  std::unordered_map<std::string, CpuDiagItemConfig> nodes_cpu_;
};

}  // namespace system_stats
}  // namespace mw