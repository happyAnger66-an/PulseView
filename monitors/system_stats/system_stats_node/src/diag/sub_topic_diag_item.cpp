...
int32_t SubTopicDiagItem::Start(const mw::proto::SystemStatsDiagConfig &cfg) {
  int32_t ret = -1;
  cfg_ = cfg;

  if (cfg.sub_topic_size() > 0) {
    auto &topics_cfg = cfg.sub_topic();
...
int32_t SubTopicDiagItem::Stop() { return 0; }

void SubTopicDiagItem::check_node_sub_topic(
...
int32_t SubTopicDiagItem::Diagnose(
    const OutputData &data,
    std::vector<diagnostic_msgs::msg::DiagnosticStatus> &status_vec) {
  for (const auto &node_sub_info : data.node_sub_infos) {
    check_node_sub_topic(node_sub_info, status_vec);
  }
  return 0;
}

REGISTER_DIAG_ITEM_CLASS(SubTopicDiagItem)
