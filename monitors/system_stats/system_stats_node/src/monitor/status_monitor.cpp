...
bool StatusMonitor::system_check(mw::proto::SystemStats &msg) {
  ...
  if (cpu_percent > system_cpu_cfg_.critical) {
    ...
    status_msg->set_error_code(mw::common::SYSTEM_CPU_CRITICAL);
    ...
    auto status_msg = msg.add_status_list();
    status_msg->set_msg(ss.str());
    status_msg->set_error_code(mw::common::SYSTEM_CPU_HIGH);
  }
  ...
  if (mem_percent > system_mem_cfg_.critical) {
    ...
    auto status_msg = msg.add_status_list();
    status_msg->set_msg(ss.str());
    status_msg->set_error_code(mw::common::SYSTEM_MEM_CRITICAL);
    ...
    auto status_msg = msg.add_status_list();
    status_msg->set_msg(ss.str());
    status_msg->set_error_code(mw::common::SYSTEM_MEM_HIGH);
  }

  return true;
}

bool StatusMonitor::node_default_cpu_check(const mw::proto::NodeInfo &node,
  ...
  if (node_cpu > default_cpu_cfg_.critical) {
    ...
    auto status_msg = msg.add_status_list();
    status_msg->set_msg(ss.str());
    status_msg->set_error_code(mw::common::NODE_CPU_CRITICAL);
    ...
    auto status_msg = msg.add_status_list();
    status_msg->set_msg(ss.str());
    status_msg->set_error_code(mw::common::NODE_CPU_HIGH);
  }

  return true;
}

bool StatusMonitor::node_default_mem_check(const mw::proto::NodeInfo &node,
                                           mw::proto::SystemStats &msg) {
  ...
  if (node_mem > default_mem_cfg_.critical) {
    ...
    auto status_msg = msg.add_status_list();
    status_msg->set_msg(ss.str());
    status_msg->set_error_code(mw::common::NODE_MEM_CRITICAL);
    ...
    auto status_msg = msg.add_status_list();
    status_msg->set_msg(ss.str());
    status_msg->set_error_code(mw::common::NODE_MEM_HIGH);
  }

  return true;
}

bool StatusMonitor::node_cpu_check(const mw::proto::NodeInfo &node,
                                   mw::proto::SystemStats &msg,
...
bool StatusMonitor::node_mem_check(const mw::proto::NodeInfo &node,
                                   mw::proto::SystemStats &msg,
                                   threshold_value &threshold) {
  float node_mem = node.mem_used_percent();
...
bool StatusMonitor::node_check(mw::proto::SystemStats &msg) {
...
bool StatusMonitor::cpu_temp_check(mw::proto::SystemStats &msg) {
  for (const auto &cpu_temp_msg : msg.cpu_temp()) {
    if (cpu_temp_msg.temperature() > cpu_temp_.critical) {
      ...
      auto status_msg = msg.add_status_list();
      status_msg->set_msg(ss.str());
      status_msg->set_error_code(mw::common::SYSTEM_CPU_TEMP_CRITICAL);
      ...
      auto status_msg = msg.add_status_list();
      status_msg->set_msg(ss.str());
      status_msg->set_error_code(mw::common::SYSTEM_CPU_TEMP_HIGH);
    }
  }
...
bool StatusMonitor::disk_usage_check(mw::proto::SystemStats &msg) {
  for (const auto &fs_info : msg.filesystem_info()) {
    if (fs_info.used_percent() > disk_usage_.critical) {
      ...
      auto status_msg = msg.add_status_list();
      status_msg->set_msg(ss.str());
      status_msg->set_error_code(mw::common::DISK_USAGE_CRITICAL);
      ...
      auto status_msg = msg.add_status_list();
      status_msg->set_msg(ss.str());
      status_msg->set_error_code(mw::common::DISK_USAGE_HIGH);
    }
  }
...
bool StatusMonitor::gpu_temp_check(const mw::proto::GpuInfo &one_gpu,
                                   mw::proto::SystemStats &msg) {
  if (one_gpu.temperature() > gpu_temp_.critical) {
    ...
    auto status_msg = msg.add_status_list();
    status_msg->set_msg(ss.str());
    status_msg->set_error_code(mw::common::GPU_TEMP_CRITICAL);
    ...
    auto status_msg = msg.add_status_list();
    status_msg->set_msg(ss.str());
    status_msg->set_error_code(mw::common::GPU_TEMP_HIGH);
  }

  return true;
}

bool StatusMonitor::gpu_pwr_check(const mw::proto::GpuInfo &one_gpu,
                                  mw::proto::SystemStats &msg) {
  if (one_gpu.pwr_usage() > gpu_pwr_.critical) {
    ...
    auto status_msg = msg.add_status_list();
    status_msg->set_msg(ss.str());
    status_msg->set_error_code(mw::common::GPU_PWR_CRITICAL);
    ...
    auto status_msg = msg.add_status_list();
    status_msg->set_msg(ss.str());
    status_msg->set_error_code(mw::common::GPU_PWR_HIGH);
  }

  return true;
}

bool StatusMonitor::gpu_mem_check(const mw::proto::GpuInfo &one_gpu,
                                  mw::proto::SystemStats &msg) {
  if (one_gpu.mem_used_percent() > gpu_mem_.critical) {
    ...
    auto status_msg = msg.add_status_list();
    status_msg->set_msg(ss.str());
    status_msg->set_error_code(mw::common::GPU_MEM_CRITICAL);
    ...
    auto status_msg = msg.add_status_list();
    status_msg->set_msg(ss.str());
    status_msg->set_error_code(mw::common::GPU_MEM_HIGH);
  }

  return true;
}

bool StatusMonitor::gpu_check(mw::proto::SystemStats &msg) {
...
int32_t StatusMonitor::RunOnce(mw::proto::SystemStats &msg) {
  system_check(msg);
  node_check(msg);
  cpu_temp_check(msg);
  gpu_check(msg);
