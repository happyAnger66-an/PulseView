...
void UdpMonitor::mon_udp() {
  while (run_flag_) {
    ...
    while (udp_proc.ReadOneLine(segs)) {
      ...
      int32_t tx_size = std::stoi(tx_rx_size[0], nullptr, 16);
      int32_t rx_size = std::stoi(tx_rx_size[1], nullptr, 16);
...
int32_t UdpMonitor::RunOnce(mw::proto::SystemStats &msg) {
  std::lock_guard<std::mutex> guard(mux_);
  for (auto udp_err : udp_errs_) {
    ...
    one_udp_msg->set_tx_size(udp_err.tx_size);
    one_udp_msg->set_rx_size(udp_err.rx_size);
  }
  udp_errs_.clear();
